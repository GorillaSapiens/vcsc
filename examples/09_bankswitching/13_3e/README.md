...
The self-test executes multiple ROM banks, switches ROM -> RAM -> RAM -> ROM,
checks two independent RAM banks and persistence, and then displays a large
`pass` or `FAIL` with small `3E` underneath from the fixed final bank.
