...
| 05 | `multicolor_full_static` | Certified scoreless 192-line static P0+P1+Ball display |
| 06 | `multicolor_score_above_static` | Static 181-line display with score above |
| 07 | `multicolor_score_below_static` | Oracle-backed faithful legacy static display with score below |
...
Examples 05 and 07 are now certified separately:

- Example 05 uses the official scoreless `player_color_192` component and is
  checked against the verified full-height smoke cartridge, exact twelve-row
  display raster, object pixels, and 262-line frames.
- Example 07 is the relocated faithful legacy cartridge and is compared
  directly with the independently built pristine BASIC 1.9 ROM: all 1,230
  visible TIA events, exact sprite rows/colors, and 264-line frames.

The remaining matrix cells are still pending one-at-a-time display repair:

| Motion | Full 192 lines | Score above | Score below |
|---|---|---|---|
| Static | 05 certified | 06 pending | 07 certified legacy baseline |
