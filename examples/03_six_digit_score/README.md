...
raster position while still totaling exactly 192 visible scanlines. Regression
tests compare every visible TIA write—address, value, scanline, and cycle—against
the pre-template cartridge and also lock stable 262-line timing.
...
at cycles `8, 16, 44, 47, 50, 53`. Horizontal positioning and `HMOVE` occur
once under VBLANK. The timed region uses documented 6502 instructions and no
nested call or stack-pointer trick.
