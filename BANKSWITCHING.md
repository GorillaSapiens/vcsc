...
0FA0, WD, 3F, 3E, 3EX, FC, F0, and E0 are fully migrated: their bank-local trampolines consume the
...
hardware already guarantees the startup mapping, as with 3F/3E/3EX/FC/F0/E0. When selector
...
Ordinary cross-bank `JMP` is not an ABI. Application/direct `JMP` targets must
remain in the same physical bank; the linker never creates mapper-changing JMP
trampolines. The only mapper-changing one-way jump generated by the toolchain is
startup/reset: a replicated vector bridge may run the mapper's `entry.s26`
normalization fragment, then performs an ordinary absolute `JMP` to the handler
in the canonical startup bank/state.
