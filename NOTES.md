...
| Packed BCD returns        | `bcd8_t` and `bcd16_t` use the normal A:X scalar return ABI; `bcd24_t` may be stored or passed but cannot be returned because scalar returns are limited to two bytes.                                                | `compiler/ABI.txt`, `compiler/compile_function.c`                                             |
