      ...
      int *swapram, uint16_t *bank_size)
{
   ...
   if (is_swapram) {
      ...
      if (!ymark)
         return 0;
      type_len = (size_t)(ymark - (tmark + 2));
      if (type_len == 0 || type_len >= type_size)
         return 0;
      memcpy(type, tmark + 2, type_len);
      type[type_len] = '\0';
      if (!parse_hex4(ymark + 2, bank_size) || ymark[6] != '\0')
         return 0;
      *read_start = 0;
      *write_start = 0;
      *has_write_start = 0;
...
static int parse_mem_declaration_metadata(const char *symbol, memory_region_t *out)
{
   ...
   if (strncmp(symbol, MEM_DECL_META_PREFIX,
      ...
      p = symbol + sizeof(MEM_DECL_META_PREFIX_V5) - 1;
                    ...
                    sizeof(MEM_DECL_META_PREFIX_V4) - 1) == 0) {
      version = 4;
      p = symbol + sizeof(MEM_DECL_META_PREFIX_V4) - 1;
   }
...
{
   return mem && !mem->swapram && !mem->data_bank_name[0] && !mem->has_write_start &&
...
//! @brief Return whether the C26 topology is Amiga Power Play FC.
static int c26_topology_is_fc(const linker_config_t *cfg)
{
   const topology_cartridge_t *cart;
   if (!cfg || cfg->topology_bank_count < 1u || cfg->topology_bank_count > 256u)
      return 0;
   cart = &cfg->topology_cartridge;
   return (cart->present_mask & 0x80u) &&
          cart->signature[0] == 'F' && cart->signature[1] == 'C' &&
          cart->signature[2] == 0 && cart->signature[3] == 0;
}

//! @brief Return whether the C26 topology is Dynacom/Megaboy F0.
static int c26_topology_is_f0(const linker_config_t *cfg)
{
   const topology_cartridge_t *cart;
   if (!cfg || cfg->topology_bank_count != 16u)
      return 0;
   cart = &cfg->topology_cartridge;
   return (cart->present_mask & 0x80u) &&
          cart->signature[0] == 'F' && cart->signature[1] == '0' &&
          cart->signature[2] == 0 && cart->signature[3] == 0;
}
...
//! @brief Return whether the C26 topology is Brazilian Fotomania 0FA0.
static int c26_topology_is_0fa0(const linker_config_t *cfg)
{
   const topology_cartridge_t *cart;
   if (!cfg || cfg->topology_bank_count != 2u)
      return 0;
   cart = &cfg->topology_cartridge;
   return (cart->present_mask & 0x80u) &&
          cart->signature[0] == '0' && cart->signature[1] == 'F' &&
          cart->signature[2] == 'A' && cart->signature[3] == '0';
}
...
//! @brief Validate Amiga Power Play FC's staged full-window topology.
static void validate_c26_fc_topology(const linker_config_t *cfg)
{
   size_t i;
   size_t startup_count = 0;
   for (i = 0; i < cfg->topology_bank_count; ++i) {
      const topology_bank_t *bank = &cfg->topology_banks[i];
      if (bank->data_only || bank->image_size != 0x1000u ||
          bank->image_offset != 0 || bank->link_start != 0xf000u ||
          bank->cpu_start != 0xf000u || bank->map_size != 0x1000u ||
          bank->has_selector) {
         fprintf(stderr,
                 "vcsc-ld: FC bank '%s' must be one selector-free canonical $F000-$FFFF 4K physical bank\n",
                 bank->name);
         exit(1);
      }
      if (!bank->has_bankcall_descriptor ||
          bank->bankcall_descriptor != (uint8_t)bank->file_index) {
         fprintf(stderr,
                 "vcsc-ld: FC bank '%s' descriptor must equal its physical/file bank index\n",
                 bank->name);
         exit(1);
      }
      if (bank->startup) {
         ...
         if (bank->file_index != 0u) {
            fprintf(stderr,
                    "vcsc-ld: FC startup/home marker must be on physical/file bank 0\n");
            exit(1);
         }
      }
   }
   if (startup_count != 1u) {
      fprintf(stderr,
              "vcsc-ld: FC topology requires physical/file bank 0 as the single startup/home bank\n");
      exit(1);
   }
}

//! @brief Validate Dynacom/Megaboy F0's increment-only full-window topology.
static void validate_c26_f0_topology(const linker_config_t *cfg)
{
   size_t i;
   size_t startup_count = 0;

   if (cfg->topology_bank_count != 16u) {
      fprintf(stderr, "vcsc-ld: F0 topology requires exactly 16 physical 4K banks\n");
      exit(1);
   }
   for (i = 0; i < cfg->topology_bank_count; ++i) {
      const topology_bank_t *bank = &cfg->topology_banks[i];
      if (bank->data_only || bank->image_size != 0x1000u ||
          ...
          bank->has_selector) {
         fprintf(stderr,
                 "vcsc-ld: F0 bank '%s' must be one selector-free canonical $F000-$FFFF 4K physical bank\n",
                 bank->name);
         exit(1);
      }
      if (!bank->has_bankcall_descriptor ||
          ...
          bank->file_index > 15u) {
         fprintf(stderr,
                 "vcsc-ld: F0 bank '%s' descriptor must equal its physical/file bank index 0..15\n",
                 bank->name);
         exit(1);
      }
      if (bank->startup) {
         startup_count++;
         if (bank->file_index != 15u) {
            fprintf(stderr,
                    "vcsc-ld: F0 startup/home marker must be on physical/file bank 15\n");
            exit(1);
         }
      }
   }
   if (startup_count != 1u) {
      fprintf(stderr,
              "vcsc-ld: F0 topology requires physical/file bank 15 as the single startup/home bank\n");
      exit(1);
   }
}
...
static void validate_c26_topology(linker_config_t *cfg)
{
   ...
   fc_profile = c26_topology_is_fc(cfg);
   f0_profile = c26_topology_is_f0(cfg);
   ...
   for (i = 0; i < cfg->topology_bank_count; ++i) {
      ...
      for (j = i + 1; j < cfg->topology_bank_count; ++j) {
         ...
         if (!e0_profile && !wd_profile && !fe_profile && !threef_family && !fc_profile && !f0_profile && !bank->has_selector && !other->has_selector &&
   ...
   if (fc_profile)
      validate_c26_fc_topology(cfg);
   if (f0_profile)
      validate_c26_f0_topology(cfg);
   ...
   for (i = 0; i < in->object_count; ++i) {
      ...
      for (j = 0; j < obj->export_count; ++j) {
         ...
         if (!mem_region_metadata_parse(sym, region, sizeof(region),
                                        ...
                                        type, sizeof(type),
                                        &declared_swapram, &declared_bank_size)) {
         ...
         if (mem->swapram != declared_swapram) {
            fprintf(stderr,
                  "vcsc-ld: mem region '%s' swapram mismatch in %s: compiler use says %s but declaration says %s\n",
                  region, obj->origin, declared_swapram ? "swapram" : "ordinary",
                  mem->swapram ? "swapram" : "ordinary");
            exit(1);
         }
         if (declared_swapram) {
            if (mem->bank_size != declared_bank_size) {
               fprintf(stderr,
                     "vcsc-ld: mem region '%s' bank_size mismatch in %s: compiler use says $%04X but declaration says $%04X\n",
                     region, obj->origin, declared_bank_size, mem->bank_size);
               exit(1);
            }
         }
         else if (mem->start != declared_read_start) {
            if (declared_split) {
               fprintf(stderr,
         ...
         if (!declared_swapram && (declared_split != mem->has_write_start ||
...
static void validate_linker_config(linker_config_t *cfg)
{
   ...
   for (i = 0; i < cfg->mem_count; ++i) {
      ...
      else if (mem->bank_size) {
         fprintf(stderr, "vcsc-ld: ordinary MEMORY region '%s' unexpectedly has bank_size=$%04X\n",
   ...
   if (c26_topology_is_fc(cfg)) {
      for (i = 0; i < cfg->bank_count; ++i) {
         ...
         if (!top || bank->size != 0x1000u || bank->start != 0xf000u ||
             bank->hotspot != 0u || !bank->has_bankcall_descriptor ||
             bank->bankcall_descriptor != (uint8_t)top->file_index ||
             bank->startup != (top->file_index == 0u)) {
            fprintf(stderr,
                    "vcsc-ld: FC bank '%s' must map $F000 with descriptor=file index and startup only on bank 0\n",
                    bank->name);
            exit(1);
         }
   ...
   } else if (c26_topology_is_f0(cfg)) {
      for (i = 0; i < cfg->bank_count; ++i) {
         cartridge_bank_t *bank = &cfg->banks[i];
         const topology_bank_t *top = NULL;
         if (bank->startup) startup_count++;
         ...
         if (!top || bank->size != 0x1000u || bank->start != 0xf000u ||
             bank->hotspot != 0u || !bank->has_bankcall_descriptor ||
             bank->bankcall_descriptor != (uint8_t)top->file_index ||
             bank->startup != (top->file_index == 15u)) {
            fprintf(stderr,
                    "vcsc-ld: F0 bank '%s' must map $F000 with descriptor=file index and startup only on bank 15\n",
                    bank->name);
            exit(1);
         }
         for (j = i + 1; j < cfg->bank_count; ++j) {
      ...
      if (startup_count != 1u) {
         fprintf(stderr, "vcsc-ld: F0 bank mapping must contain exactly one startup bank\n");
         exit(1);
      }
   } else if (c26_topology_is_3f(cfg) || c26_topology_is_3e(cfg) || c26_topology_is_3ex(cfg)) {
      size_t final_index = cfg->topology_bank_count - 1u;
      for (i = 0; i < cfg->bank_count; ++i) {
         cartridge_bank_t *bank = &cfg->banks[i];
         const topology_bank_t *top = NULL;
...
//! @brief Return whether one object fits wholly inside one physical swapram bank.
static int memory_region_object_bank_constraints_hold(const memory_region_t *mem,
                                                      uint32_t addr,
                                                      uint16_t size)
{
   uint32_t offset;

   if (!mem || !mem->swapram || size == 0)
      return 1;
   if (!mem->bank_size || size > mem->bank_size || addr < mem->start)
      return 0;
   offset = addr - mem->start;
   if (offset + size > mem->size)
      return 0;
   return (offset % mem->bank_size) + size <= mem->bank_size;
}

//! @brief Advance a failed swapram candidate directly to the next bank boundary.
static uint32_t memory_region_next_bank_candidate(const memory_region_t *mem,
                                                  uint32_t addr)
{
   uint32_t offset;
   uint32_t next;

   if (!mem || !mem->swapram || !mem->bank_size || addr < mem->start)
      return addr + 1u;
   offset = addr - mem->start;
   next = mem->start + ((offset / mem->bank_size) + 1u) * mem->bank_size;
   return next > addr ? next : addr + 1u;
}
   ...
   const object_layout_t *constraints, const char *what, const char *origin)
{
   ...
   if (size == 0)
      return cursor->cur;
   if (mem && mem->swapram && size > mem->bank_size) {
      fprintf(stderr,
              "vcsc-ld: swapram object %s from %s is $%04X bytes; region '%s' bank_size is only $%04X\n",
              what, origin, size, mem->name, mem->bank_size);
      exit(1);
   }
   ...
   while (!object_page_constraints_hold(cfg, constraints, addr) ||
          !memory_region_object_bank_constraints_hold(mem, addr, size)) {
      uint32_t next = memory_region_object_bank_constraints_hold(mem, addr, size)
         ? addr + 1u : memory_region_next_bank_candidate(mem, addr);
      addr = align_up_phase_u32(next, alignment, phase);
...
static uint16_t generic_bankcall_selector_base(const linker_config_t *cfg)
{
   ...
   int fc = c26_topology_is_fc(cfg);
   int f0 = c26_topology_is_f0(cfg);
   ...
   if (fc) {
      for (i = 0; i < cfg->topology_bank_count; ++i) {
         ...
         if (!bank || bank->data_only || bank->has_selector ||
             ...
             bank->bankcall_descriptor != (uint8_t)i) {
            fprintf(stderr,
                    "vcsc-ld: FC inline-target calls require descriptor equal to physical/file bank index 0..N-1\n");
            exit(1);
         }
      }
      return 0u; /* FC stages descriptor bits explicitly; no indexed selector base. */
   }

   if (f0) {
      if (cfg->topology_bank_count != 16u) {
         fprintf(stderr, "vcsc-ld: F0 inline-target calls require exactly 16 banks\n");
         exit(1);
      }
      for (i = 0; i < cfg->topology_bank_count; ++i) {
         const topology_bank_t *bank = c26_topology_bank_by_file_index(cfg, (uint16_t)i);
         if (!bank || bank->data_only || bank->has_selector ||
             ...
             bank->bankcall_descriptor != (uint8_t)i) {
            fprintf(stderr,
                    "vcsc-ld: F0 inline-target calls require descriptor equal to physical/file bank index 0..15\n");
            exit(1);
         }
      }
      return 0u; /* F0 advances relative to source/target descriptors; no absolute selector base. */
   }
   ...
   if (m0fa0) {
      const topology_bank_t *file0 = c26_topology_bank_by_file_index(cfg, 0u);
      const topology_bank_t *file1 = c26_topology_bank_by_file_index(cfg, 1u);
...
static uint16_t generic_bankcall_reserved_size(const linker_config_t *cfg)
{
   ...
   if (c26_topology_is_f0(cfg))
      return VCSC_F0_BANKCALL_RESERVED_SIZE;
   if (bankcall_descriptor_abi_enabled(cfg))
...
#define ENTRY(name) do { if (size) *size = vcsc_##name##_entry_size; return vcsc_##name##_entry; } while (0)
   ...
   if (SIG4('3','E','X',0))
      ENTRY(m3ex);
   if (SIG4('F','C',0,0) || (!has_sig && cfg && str_ieq(cfg->mapper, "FC")))
      ENTRY(fc);
                                          ...
                                          uint16_t ptr0)
{
   ...
   if (c26_topology_is_0840(cfg)) {
   ...
   else if (c26_topology_is_0fa0(cfg)) {
      instantiate_bankcall_template(table,
         vcsc_m0fa0_bankcall_template,
         VCSC_M0FA0_BANKCALL_TEMPLATE_SIZE,
         VCSC_M0FA0_BANKCALL_RESERVED_SIZE,
         vcsc_m0fa0_bankcall_ptr_patches,
      ...
      instantiate_bankcall_template(table,
         vcsc_fc_bankcall_template,
         VCSC_FC_BANKCALL_TEMPLATE_SIZE,
         VCSC_FC_BANKCALL_RESERVED_SIZE,
         vcsc_fc_bankcall_ptr_patches,
         VCSC_FC_BANKCALL_PTR_PATCH_COUNT,
         vcsc_fc_bankcall_selector_patches,
         VCSC_FC_BANKCALL_SELECTOR_PATCH_COUNT,
         vcsc_fc_bankcall_source_descriptor_patches,
         VCSC_FC_BANKCALL_SOURCE_DESCRIPTOR_PATCH_COUNT,
         source_descriptor,
         VCSC_FC_BANKCALL_SWITCH_OFFSET,
         VCSC_FC_BANKCALL_INTERNAL_JSR_OPERAND_OFFSET,
         selector_base, canonical_base, ptr0);
   }
   else if (c26_topology_is_f0(cfg)) {
      instantiate_bankcall_template(table,
         vcsc_f0_bankcall_template,
         VCSC_F0_BANKCALL_TEMPLATE_SIZE,
         VCSC_F0_BANKCALL_RESERVED_SIZE,
         vcsc_f0_bankcall_ptr_patches,
         VCSC_F0_BANKCALL_PTR_PATCH_COUNT,
         vcsc_f0_bankcall_selector_patches,
         VCSC_F0_BANKCALL_SELECTOR_PATCH_COUNT,
         vcsc_f0_bankcall_source_descriptor_patches,
         VCSC_F0_BANKCALL_SOURCE_DESCRIPTOR_PATCH_COUNT,
         source_descriptor,
         VCSC_F0_BANKCALL_SWITCH_OFFSET,
         VCSC_F0_BANKCALL_INTERNAL_JSR_OPERAND_OFFSET,
         selector_base, canonical_base, ptr0);
   }
...
//! @brief Resolve the runtime MEMORY region selected by one ordinary data layout rule.
static const memory_region_t *find_runtime_memory_for_layout(const linker_config_t *cfg,
                                                             const object_layout_t *lay)
{
   const segment_rule_t *fallback;
   const segment_rule_t *rule;
   ...
   if (!cfg || !lay)
      return NULL;
   if (lay->segid == O26_SEG_DATA)
      fallback = find_segment_rule(cfg, "DATA");
   else if (lay->segid == O26_SEG_BSS)
      fallback = find_segment_rule(cfg, "BSS");
   else if (lay->segid == O26_SEG_ZP)
      fallback = find_segment_rule(cfg, "ZEROPAGE");
   else
      return NULL;
   rule = find_layout_segment_rule(cfg, lay->name, fallback);
   run_name = rule_run_region_name(rule ? rule : fallback);
   return run_name && *run_name ? find_memory(cfg, run_name) : NULL;
}
...
static uint32_t memory_region_runtime_used_bytes(const linker_config_t *cfg,
                                                 const memory_region_t *mem,
                                                 const input_set_t *in)
{
   uint8_t *occupied;
                            ...
                            const char *indent)
{
   ...
   for (i = 0; i < cfg->mem_count; ++i) {
      ...
      if (!memory_region_is_writable_ram(mem))
         continue;
      object_bytes = memory_region_runtime_used_bytes(cfg, mem, in);
                           ...
                           const layout_t *layout, const link_image_t *image)
{
   ...
   for (i = 0; i < cfg->mem_count; ++i) {
      ...
      else if (cfg->mem[i].has_write_start) {
   ...
   for (i = 0; i < in->object_count; ++i) {
      ...
      for (j = 0; j < o->layout_count; ++j) {
         ...
         if (lay->segid == O26_SEG_TEXT) {
            ...
            const memory_region_t *runtime_mem = find_runtime_memory_for_layout(cfg, lay);
            if (!runtime_mem)
            ...
            if (runtime_mem && !runtime_mem->swapram && runtime_mem->has_write_start)
               fprintf(fp, " write=$%04X",
                       memory_runtime_write_address(cfg, runtime_mem->name,
                                                    lay->run_addr, lay->size));
            if (runtime_mem && runtime_mem->swapram)
               fprintf(fp, " size=$%04X load-page=%s",
