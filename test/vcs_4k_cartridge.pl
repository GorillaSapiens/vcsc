...
my $source = File::Spec->catfile($repo, 'examples', 'vcs', 'solid_color.n');
...
$map_text =~ /__stack_top\s+\$00DF/
   or die "map does not reserve the hardware-stack window above \$00DF\n";
$map_text =~ /\bmain\b/
