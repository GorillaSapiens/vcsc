...
my $nlib = File::Spec->catdir($repo, 'libraries', 'nlib');
my $archive = File::Spec->catfile($nlib, 'nlib.a65');
...
-f File::Spec->catfile($nlib, 'asm', 'handler.asm')
   and die "obsolete default interrupt handlers remain\n";
-f File::Spec->catfile($nlib, 'nrt0_noint.s')
   and die "obsolete separate vector-stub source remains\n";

my $startup = slurp(File::Spec->catfile($nlib, 'nrt0.s'));
