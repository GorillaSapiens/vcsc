...
sub profile_from_source {
   ...
   return 'f0' if $text =~ /^\s*include\s+"F0\/mapper\.c26"\s*$/m;
...
for my $source (@example_sources) {
   ...
   # Generated font subsets are data fragments even when a generator consumes
   # them instead of a C26 include. Never treat them as cartridge entry points.
   next if $source_text =~ m{^// Generated by libraries/vcs/fonts/(?:make_font_subset|make_message_glyphs)\.pl; do not edit\.}m;
...
for my $entry (@examples) {
   ...
   my %known_signature=map { $_=>1 } (
      ...
      "F4\0\0", "F4SC", "FA\0\0", "FA2\0", "CV\0\0", "OMNI", "JANE", "0840", "UA\0\0", "UASW", "0FA0", "E0\0\0", "FE\0\0", "WD\0\0", "3F\0\0", "3E\0\0", "FC\0\0", "F0\0\0", "DPC\0",
   );
