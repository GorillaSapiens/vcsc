...
for my $entry (@examples) {
   ...
   } elsif ($profile eq 'cv' || $profile eq '4ksc' || $profile eq 'f8sc' || $profile eq 'fa' || $profile eq 'omni') {
   ...
   my $expected_size = ($file eq 'bankswitching_diagnostic.c26' ||
      ...
      : ($profile eq '2k' || $profile eq 'cv') ? 2048 : ($profile eq 'f8' || $profile eq 'f8sc') ? 8192 : $profile eq 'fa' ? 12288 : $profile eq 'omni' ? 32768 : 4096;
   ...
   my %known_signature=map { $_=>1 } (
      ...
      "F4\0\0", "F4SC", "FA\0\0", "CV\0\0", "OMNI",
   );
