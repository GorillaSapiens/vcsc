...
for my $entry (@examples) {
   ...
   } elsif ($profile eq '4ksc' || $profile eq 'f8sc' || $profile eq 'fa' || $profile eq 'omni') {
   ...
   my $expected_size = ($file eq 'bankswitching_diagnostic.c26' ||
      ...
      : $profile eq '2k' ? 2048 : ($profile eq 'f8' || $profile eq 'f8sc') ? 8192 : $profile eq 'fa' ? 12288 : $profile eq 'omni' ? 32768 : 4096;
