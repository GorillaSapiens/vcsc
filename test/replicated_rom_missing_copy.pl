...
my $profile = File::Spec->catfile($vcs, 'F6/mapper.c26');
...
require_ok('build F6 replicated function fallback', $driver, '-I', $vcs, '-DVCS_NO_DEFAULT_ROM',
           '-Map', $good_map_path, '-o', $good_bin, $good_src);
...
for my $start_bank (0 .. 3) {
   my ($dump, $err) = require_ok("simulate F6 fallback from bank $start_bank",
      $sim, '--map', $good_map_path, "--start-bank=$start_bank",
...
SRC
require_ok('build three-copy F6 image', $driver, '-I', $vcs, '-DVCS_NO_DEFAULT_ROM',
...
my ($three_dump, $three_err) = require_ok('simulate three-copy F6 image',
   $sim, '--map', $three_map_path, '--start-bank=3', sprintf('--stop-pc=0x%04X', $done3),
...
my $err = require_fail('link pinned reader without a local object copy',
   ...
   $driver, '-I', $vcs, '-DVCS_NO_DEFAULT_ROM', '-o', $bad_bin, $bad_src);
