...
# phase: e2e
# timeout: 45
# expectstdout: vcs_all_five_interactive_examples ok: three all-five interactive renderer examples pass build, frame, five-object controls, filtered score controls, endpoints, and reset checks
# expectexit: 0
...
my @cases=(
 { dir=>'05_all_five_192/01_interactive', stem=>'all_five_192_interactive', profile=>'all5_192', score=>0 },
 { dir=>'06_all_five_181/01_score_above/01_interactive', stem=>'all_five_181_score_above_interactive', profile=>'all5_above', score=>1 },
 { dir=>'06_all_five_181/02_score_below/01_interactive', stem=>'all_five_181_score_below_interactive', profile=>'all5_below', score=>1 },
);
...
for my $case (@cases) {
   ...
   $text =~ /renderers\/all_five_(?:181|192)\/all_five_(?:181|192)\.c26/
      or die "$dir does not use an all-five renderer\n";
   ...
   ($rc,$sig,$out,$err)=capture($driver,'-I',$vcs,'-Map',$mapfile,$src,'-o',$bin);
...
print "vcs_all_five_interactive_examples ok: three all-five interactive renderer examples pass build, frame, five-object controls, filtered score controls, endpoints, and reset checks\n";
