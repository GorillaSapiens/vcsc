...
# phase: e2e
# expectstdout: vcs_all_five_stripes_192 ok
# expectexit: 0
...
my $source=File::Spec->catfile($repo,qw(test fixtures all_five_stripes_192 smoke.c26));
my $bin=File::Spec->catfile($tmp,'all_five_stripes_192.bin');
my $mapfile=File::Spec->catfile($tmp,'all_five_stripes_192.map');
...
$map =~ /^\s+RODATA\.__vcsc_object\$game_playfield_data\s+load=\$[0-9A-Fa-f]{4}\s+size=\$0006\b/m
   or die "stripe playfield payload is not six bytes\n";
...
my @checks=(
   ...
   ['phase','vcs_playfield_phase.cpp',[12,12,40,'all-five-stripes-192'],
      "vcs_playfield_stripes_192 ok: 12 rows x 16 lines with stable six-write phases\n"],
...
print "vcs_all_five_stripes_192 ok\n";
