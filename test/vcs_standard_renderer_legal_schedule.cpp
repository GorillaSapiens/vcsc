//! @file vcs_standard_renderer_legal_schedule.cpp
...
[[noreturn]] void fail(const char *message) {
   std::fprintf(stderr, "vcs_standard_renderer_legal_schedule: %s\n", message);
   std::exit(1);
}
...
void expect_line(const std::map<uint64_t, std::vector<MaskEvent>> &by_line,
   ...
   for (size_t i = 0; i < expected.size(); ++i) {
      if (found->second[i].cycle != expected[i].first ||
         ...
         std::fprintf(stderr,
            "vcs_standard_renderer_legal_schedule: line %llu event %zu is zp $%02x "
...
int main(int argc, char **argv) {
   ...
   for (const auto &entry : final_values) {
      ...
      if (actual != entry.second) {
         std::fprintf(stderr,
            "vcs_standard_renderer_legal_schedule: final mask +%u is $%02x; expected $%02x\n",
   ...
   std::printf(
      "vcs_standard_renderer_legal_schedule ok: 46 scanlines, three steady masks and five final values locked\n");
   return 0;
}
