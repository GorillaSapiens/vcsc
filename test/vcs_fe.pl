...
$mk_flat =~ /^\s*stella\s+-dev\.tv\.jitter\s+0\s+-basedir\s+"\$\(CURDIR\)"\s+-userdir\s+"\$\(CURDIR\)"\s+-bs\s+FE\s+"\$\(CURDIR\)\/\$\(TARGET\)"\s*$/m
...
if ($stella_mode) {
   ...
   my $xvfb=find_executable($ENV{VCSC_XVFB} || $ENV{XVFB} || 'Xvfb') or die "FE Stella certification requires Xvfb\n";
