...
# phase: e2e
# expectstdout: vcs_big_font_family ok: decimal and hex are exact 16-row subsets of big ASCII and stay page-contained
# expectexit: 0
...
for my $spec ([$dec_text,160,'decimal'],[$hex_text,256,'hex']) {
   ...
   $text =~ /align\s*\(\s*256\s*\)\s+const\s+uint8_t\s+score_font\s*\[\s*\Q$size\E\s*\]\s*:=/
      or die "big $name is not a 256-aligned $size-byte table\n";
}
for my $digit (0..9) {
...
for my $variant (['decimal',160],['hex',256]) {
   ...
   $map_text =~ /^\s+RODATA\.__vcsc_object\$score_font\s+load=\$([0-9A-Fa-f]{4})\s+size=\$\Q$hexsize\E\b.*component-align=\$0100/m
      or die "big $name linked table does not have expected size/alignment\n";
   (hex($1)&0xff)==0 or die "big $name table is not page aligned\n";
}

print "vcs_big_font_family ok: decimal and hex are exact 16-row subsets of big ASCII and stay page-contained\n";
