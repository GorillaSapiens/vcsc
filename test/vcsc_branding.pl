...
my @developer_records_extra = sort grep {
   ...
   $_ ne 'context-history' && $_ ne 'enhanced.txt' && $_ ne 'enhanced_asymmetric.txt' && $_ ne 'bankswitching.txt' && $_ ne 'ram_optimization.txt' && $_ ne 'inline_roadmap.txt' && $_ ne 'video_standard_roadmap.txt' && $_ ne 'instruction.txt' && $_ ne 'disassembler.txt'
