...
my @developer_records_extra = sort grep {
   ...
   $_ ne 'context-history' && $_ ne 'release.txt' && $_ ne 'enhanced_asymmetric.txt' && $_ ne 'bankswitching.txt' && $_ ne 'instruction.txt' && $_ ne 'disassembler.txt' && $_ ne 'heart_score.txt' && $_ ne 'DRA.txt' && $_ ne 'example_reorg.txt' && $_ ne '3ex.txt'
