...
constexpr std::array<std::array<uint8_t,8>,10> kPairFont{{
   {{0x6,0x9,0x9,0x9,0x9,0x9,0x9,0x6}},
   {{0x2,0x6,0x2,0x2,0x2,0x2,0x2,0x7}},
   {{0x6,0x9,0x1,0x2,0x4,0x8,0x8,0xf}},
   {{0xe,0x1,0x1,0x6,0x1,0x1,0x9,0x6}},
   {{0x2,0x6,0xa,0xa,0xf,0x2,0x2,0x2}},
   {{0xf,0x8,0x8,0xe,0x1,0x1,0x9,0x6}},
   {{0x6,0x8,0x8,0xe,0x9,0x9,0x9,0x6}},
   {{0xf,0x1,0x2,0x2,0x4,0x4,0x4,0x4}},
   {{0x6,0x9,0x9,0x6,0x9,0x9,0x9,0x6}},
   {{0x6,0x9,0x9,0x7,0x1,0x1,0x1,0x6}}
}};
...
void verify_six_schedule(const FrameTrace&f,uint64_t e,const std::string&kind){
   ...
   if(kind=="center"){
      ...
      require_write(f,e,10,kResp0,3,"RESP0");require_write(f,e,13,kResp1,3,"RESP1");require_write(f,e,18,kColup0,0x0e,"COLUP0");require_write(f,e,21,kColup1,0x0e,"COLUP1");
      require_write(f,e,24,kHmclr,0x0e,"HMCLR");require_write(f,e,29,kHmp0,0x30,"HMP0");require_write(f,e,34,kHmp1,0xb0,"HMP1");
   }else{
      require_write(f,e,8,kColup0,0x0e,"COLUP0");require_write(f,e,11,kColup1,0x0e,"COLUP1");require_write(f,e,14,kHmclr,0x0e,"HMCLR");
      require_write(f,e,19,kHmp0,0xc0,"HMP0");require_write(f,e,24,kHmp1,0xd0,"HMP1");require_write(f,e,49,kResp0,0xd0,"RESP0");require_write(f,e,52,kResp1,0xd0,"RESP1");
   }
   require_write(f,e,71,kHmove,kind=="left"?0xb0:kind=="right"?0xd0:0x90,"HMOVE");
...
uint8_t pair_row(uint8_t packed,unsigned row){return static_cast<uint8_t>((kPairFont[(packed>>4)&15][row]<<4)|kPairFont[packed&15][row]);}
