...
$source_text =~ /WSYNC\s*:=\s*\$A\s*;\s*VSYNC\s*:=\s*2\s*;\s*WSYNC\s*:=\s*\$A\s*;\s*WSYNC\s*:=\s*\$A\s*;\s*WSYNC\s*:=\s*\$A\s*;\s*VSYNC\s*:=\s*0\s*;/s
...
$source_text =~ /TIM64T\s*:=\s*42\s*;.*?while\s*\(\s*!\s*\(\s*TIMINT\s*&\s*0x80\s*\)\s*\).*?WSYNC\s*:=\s*\$A\s*;\s*VBLANK\s*:=\s*0\s*;/s
   or die "Ode to Joy lost timer-owned VBLANK deadline/alignment\n";
$source_text =~ /TIM64T\s*:=\s*35\s*;\s*music_tick\(\);.*?WSYNC\s*:=\s*\$A\s*;\s*while\s*\(\s*!\s*\(\s*TIMINT\s*&\s*0x80\s*\)\s*\).*?WSYNC\s*:=\s*\$A\s*;/s
