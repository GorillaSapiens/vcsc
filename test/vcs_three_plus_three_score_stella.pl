#!/usr/bin/perl
# Authoritative Stella 7.0 visible-raster certification for the 3+3 score.
use strict;
use warnings;
use Cwd qw(abs_path);
...
ok('build three-plus-three score',$driver,'-I',$vcs,$source,'-o',$rom);
...
$ae eq ''&&$we eq '' or die $ae.$we; $actual eq $wanted or die "three-plus-three Stella raster differs: actual=$actual reference=$wanted";
chomp $actual; print "Stella three-plus-three score raster passed: $actual\n";
