...
for my $name (qw(F8 F6 F4 F8SC)) {
   ...
   require_re($map,qr/region=ram depth=3 bytes=\$000C .*extra=\$0004/,
      "$name hardware-stack accounting is not 12 bytes with four hidden bytes");
}

# Lock the measured post-optimization ROM and bridge costs for this diagnostic.
for my $check (
   ['F8',1891,43,30], ['F6',1891,43,60], ['F4',1891,43,120], ['F8SC',1891,53,30]
) {
