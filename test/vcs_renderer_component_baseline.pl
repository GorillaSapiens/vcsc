...
for my $p (@profiles) {
   ...
   my $renderer=read_file(File::Spec->catfile($dir,"$p->{dir}_renderer.s26"));
   ...
   require_re($renderer,qr/^\.importzp\s+\Q$p->{score}\E\s*$/m,
      "$p->{dir}: renderer no longer imports predecessor score");
   require_re($renderer,qr/^\.importzp\s+\Q$p->{color}\E\s*$/m,
      "$p->{dir}: renderer no longer imports predecessor score color");
   require_re($renderer,qr/^\.export\s+\Q$p->{table}\E\s*$/m,
      "$p->{dir}: embedded score table export changed");
   for my $register (qw(VSYNC VBLANK WSYNC TIM64T)) {
      require_re($renderer,qr/^\Q$register\E\s*=\s*\$/m,
...
require_re($unofficial,qr/zero-byte result is intentional evidence.*not a failed optimization/si,
   'unofficial-opcode comparison incorrectly assumes a saving');
require_re($doc,qr/official linked ROM bytes:\s+2081.*unofficial linked ROM bytes:\s+2081.*signed saving:\s+0/s,
...
require_re($unofficial,qr/official linked ROM bytes:\s+2081.*unofficial linked ROM bytes:\s+2081.*signed byte difference:\s+0/s,
