...
require_re($text,qr/^include "frame_ntsc\.c26"$/m,
   'example 06 does not use the NTSC frame scheduler');
...
@visual>=16 or die "example 06 lacks sixteen visual sprite rows\n";
for my $name (qw(
   ...
   $text =~ /\b\Q$name\E\b/ or die "example 06 does not use $name\n";
}

my($rc,$sig,$out,$err)=capture(
   $driver,'-I',$vcs,'-T',$cfg,'-Map',$mapfile,$source,'-o',$bin);
$rc==0 && !$sig or die "example 06 build failed\n$out$err";
without_usage($out) eq '' && $err eq '' or die "example 06 build wrote output\n$out$err";
length(read_file($bin))==4096 or die "example 06 ROM is not 4096 bytes\n";
...
$rc==0 && !$sig or die "example 06 pixel harness build failed\n$out$err";
$out eq '' && $err eq '' or die "example 06 pixel harness build wrote output\n$out$err";
...
$rc==0 && !$sig or die "example 06 pixel raster failed\n$out$err";
