...
require_re($text,qr/^include "frame_ntsc\.c26"$/m,
   'player_color_192 static example does not use the NTSC frame scheduler');
require_re($text,qr/^include "color_ntsc\.c26"$/m,
   'player_color_192 static example does not use named NTSC colors');
require_re($text,qr/^include "playfield\.c26"$/m,
   'player_color_192 static example does not use visual playfield rows');
require_re($text,qr/template "renderers\/player_color_192\/player_color_192\.c26" as game/,
   'player_color_192 static example does not use the full 192-line player-color renderer');
$text !~ /six_glyph_component|score_draw|score_/ or die "player_color_192 static example unexpectedly contains score ownership\n";
my @pfrows=$text =~ /VCS_PLAYFIELD_ROW\s*\(/g;
@pfrows==12 or die "player_color_192 static example has ".scalar(@pfrows)." playfield rows, expected 12\n";
my @visual=$text =~ /0b[.X]{8}(?![.X])/g;
@visual>=16 or die "player_color_192 static example lacks sixteen visual sprite rows\n";
for my $name (qw(
   ...
   $text =~ /\b\Q$name\E\b/ or die "player_color_192 static example does not use $name\n";
}

my($rc,$sig,$out,$err)=capture(
   $driver,'-I',$vcs,'-T',$cfg,'-Map',$mapfile,$source,'-o',$bin);
$rc==0 && !$sig or die "player_color_192 static example build failed\n$out$err";
without_usage($out) eq '' && $err eq '' or die "player_color_192 static example build wrote output\n$out$err";
length(read_file($bin))==4096 or die "player_color_192 static example ROM is not 4096 bytes\n";
...
$rc==0 && !$sig or die "player_color_192 static example pixel harness build failed\n$out$err";
$out eq '' && $err eq '' or die "player_color_192 static example pixel harness build wrote output\n$out$err";
...
$rc==0 && !$sig or die "player_color_192 static example pixel raster failed\n$out$err";
$out eq "vcs_player_color_192 static ok: exact 192-line frame, VBLANK positioning, P0/P1 rows, Ball, and no missiles\n"
   or die "unexpected player_color_192 static example pixel output: $out";
$err eq '' or die "player_color_192 static example pixel stderr: $err";
...
$rc==0 && !$sig or die "player_color_192 static example display harness build failed\n$out$err";
$out eq '' && $err eq '' or die "player_color_192 static example display harness build wrote output\n$out$err";
...
$rc==0 && !$sig or die "player_color_192 static example display raster failed\n$out$err";
$out eq "vcs_multicolor_display_raster full ok: exact PF rows, glyph bytes/colors, Ball, and score ownership\n"
   or die "unexpected player_color_192 static example display output: $out";
$err eq '' or die "player_color_192 static example display stderr: $err";

print "vcs_multicolor_full_static ok: player_color_192 static example builds a 262-line cartridge with exact asymmetric playfield rows, P0/P1 colors, Ball, and no missiles\n";
