...
my @mapper = qw(F8 F8SC F6 F6SC F4 F4SC FA DPC FA2 JANE 0840 UA UASW 0FA0 WD 3F 3E 3EX FC F0);
...
for my $mapper (@mapper) {
   ...
   if ($mapper eq '3F' || $mapper eq '3E' || $mapper eq '3EX' || $mapper eq 'FC' || $mapper eq 'F0') {
