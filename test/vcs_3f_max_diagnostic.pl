...
my %sym=map { $_=>map_symbol($map,$_) } qw(draw_result failure torture_count);
my($sim_out,$sim_err)=require_ok('simulate complete 3F max torture',$sim,'--map',$map_path,
   sprintf('--stop-pc=0x%04X',$sym{draw_result}),'--dump-on-stop',$bin);
...
my($timing_out,$timing_err)=require_ok('time 3F max visible frame',$timing,$bin,'50','--no-audio','--raw-lines','264');
$timing_out eq "vcs_frame_timing ok: 47 frames at 262 lines, 0 AUDV0 writes\n"
   or die "3F max frame timing was not exactly 262 scanlines:\n$timing_out";
