...
# phase: e2e
# expectstdout: vcs_all_five_stripes2_tail2_192 ok
# expectexit: 0
...
my $bin=File::Spec->catfile($tmp,'all_five_stripes2_tail2_192.bin');
my $mapfile=File::Spec->catfile($tmp,'all_five_stripes2_tail2_192.map');
my($rc,$sig,$out,$err)=capture($driver,'-I',$vcs,'-DVCS_STRIPES2_CERT_TAIL2','-Map',$mapfile,$source,'-o',$bin);
...
substr($rom,$height_addr-0xf000,2) eq pack('C*',50,46)
   or die "generated stripe pair heights are not 50/46\n";
substr($rom,$schedule_addr-0xf000,4) eq pack('C*',6,2,5,6)
   or die "generated coarse/tail stripe schedule is not {6,2,5,6}\n";
...
my @checks=(
   ...
   ['phase','vcs_playfield_phase.cpp',[12,12,40,'all-five-stripes2-tail2-192'],
      "vcs_playfield_stripes2_tail2_192 ok: exact 100/92 data+color boundary with stable six-write phases\n"],
...
print "vcs_all_five_stripes2_tail2_192 ok\n";
