...
const std::array<std::array<uint8_t,8>,4> kInteractiveP0{{
...
const std::array<std::array<uint8_t,8>,4> kInteractiveP1{{
...
std::array<uint8_t,8> selected_interactive_sprite(bool player1) {
   const size_t player=player1 ? 1 : 0;
   const uint8_t x=memory_image[static_cast<uint8_t>(object_x_zp+player)];
   const uint8_t y=memory_image[y_zp[player]];
   const size_t frame_index=static_cast<size_t>((x^y)&3);
   return player1 ? kInteractiveP1[frame_index] : kInteractiveP0[frame_index];
}
void verify_object_pixel_raster() {
   ...
   const std::array<uint8_t,8> p0 = interactive_artwork
      ? selected_interactive_sprite(false)
   ...
   const std::array<uint8_t,8> p1 = interactive_artwork
      ? selected_interactive_sprite(true)
      : (score_order==ScoreOrder::None
