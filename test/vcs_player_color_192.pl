...
($rc,$sig,$out,$err)=capture($driver,'-I',$vcs,'-Map',$terminal_mapfile,$terminal_source,'-o',$terminal_bin);
...
for my $edge (['ball-top',0,'top edge'],['ball-row-edge',8,'first row boundary']) {
   ...
   ($rc,$sig,$out,$err)=capture($driver,'-I',$vcs,'-Map',$edge_mapfile,$edge_src,'-o',$edge_bin);
...
for my $phase (qw(init vblank draw overscan)) {
   ...
   ($rc,$sig,$out,$err)=capture($driver,'-I',$vcs,$badsrc,'-o',$badbin);
