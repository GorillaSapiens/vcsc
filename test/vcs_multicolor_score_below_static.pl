...
my $source=File::Spec->catfile($repo,qw(examples 07_multicolor_score_below_static multicolor_score_below_static.c26));
my $oracle=File::Spec->catfile($repo,qw(test oracles pristine_basic_v1.9_playercolors faithful_legacy_playercolors.bin));
my $bin=File::Spec->catfile($tmp,'multicolor_score_below_static.bin');
my $map=File::Spec->catfile($tmp,'multicolor_score_below_static.map');

my $text=read_file($source);
require_re($text,qr/^include "color_ntsc\.c26"$/m,
   'example 07 does not use named NTSC colors');
require_re($text,qr/^include "playfield\.c26"$/m,
   'example 07 does not use visual playfield rows');
require_re($text,qr/template "kernels\/faithful_legacy_playercolors\/faithful_legacy_playercolors\.c26" as legacy/,
   'example 07 does not use the oracle-backed faithful legacy kernel');
require_re($text,qr/^uint8_t legacy_playfield\[48\]/m,
   'example 07 playfield is not RAM-backed');
my @pfrows=$text =~ /VCS_PLAYFIELD_ROW\s*\(/g;
@pfrows==12 or die "example 07 has ".scalar(@pfrows)." playfield rows, expected 12\n";
my @visual=$text =~ /0b[.X]{8}(?![.X])/g;
@visual>=16 or die "example 07 lacks sixteen visual sprite rows\n";
for my $name (qw(
   ...
   $text =~ /\b\Q$name\E\b/ or die "example 07 does not use $name\n";
}

my($rc,$sig,$out,$err)=capture(
   $driver,'-I',$vcs,'-Wa,--illegals','-T',$cfg,'-Map',$map,$source,'-o',$bin);
$rc==0 && !$sig or die "example 07 build failed\n$out$err";
without_usage($out) eq '' && $err eq '' or die "example 07 build wrote output\n$out$err";
length(read_file($bin))==4096 or die "example 07 did not produce a 4096-byte ROM\n";
...
my $harness=File::Spec->catfile($tmp,'vcs_multicolor_score_below_static_compare');
...
$rc==0 && !$sig or die "example 07 differs from pristine BASIC oracle\n$out$err";
...
$rc==0 && !$sig or die "example 07 sprite oracle failed\n$out$err";
...
print "vcs_multicolor_score_below_static ok: public example 07 matches pristine BASIC oracle: 1230 visible events, 264-line frames, exact sprites\n";
