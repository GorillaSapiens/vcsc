...
[[noreturn]] void fail(const std::string &message) {
   std::fprintf(stderr, "vcs_frame_50hz_scheduler: %s\n", message.c_str());
   std::exit(1);
}
...
void check_mode(const std::string &mode, uint16_t flag_address) {
   uint64_t expected_lines = 314;
   uint8_t expected_flags = 0;
   uint64_t clear_offset = 48;
   uint64_t overscan_offset = 276;
   ...
   for (size_t i = 2; i + 1 < vsync_assertions.size(); ++i) {
      ...
      for (const auto &event : timer_writes) {
         ...
         if (loads == 1 && event.value != 52) fail("wrong VBLANK TIM64T preload");
         if (loads == 2 && event.value != 41) fail("wrong overscan TIM64T preload");
      }
...
int main(int argc, char **argv) {
   ...
   std::printf("vcs_frame_50hz_scheduler %s ok\n", mode.c_str());
   return 0;
}
