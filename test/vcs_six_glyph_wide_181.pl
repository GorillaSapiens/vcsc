...
for my $case (
   ...
   $source_text =~ /instantiate\s+"renderers\/all_five\/all_five\.c26"\s+as\s+game\s*\(lines:=181,\s*missiles:=0,\s*player_colors:=1\)/
   ...
   $map =~ /rom\s+used=2729 bytes/ or die "$tag ROM accounting changed\n";
   $map =~ /ram\s+used=52 bytes.*objects=48 bytes hardware-stack=4 bytes/
