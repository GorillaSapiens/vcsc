...
Tests without an explicit `timeout:` header use a 45-second per-case timeout; long-running cases can override it in their header.
...
possible initially selected bank. For the F8/F6/F4 profiles it locks the resulting eighteen-byte bridge
(three-byte mapper-owned `entry.s26` plus `JMP handler` per slot),
...
from `examples/07_diagnostics/bankswitching/f864/bankswitching_diagnostic.c26`. The diagnostic composes
...
compact `$44-$6C` mirror clear of the `$04-$2C` non-beam TIA registers in all three stock startup bodies, and omission of
...
visible borders. `vcs_video_standard_228_matrix.pl` locks one native PAL and one native SECAM
example for every line-parameterized renderer that advertises a 228 profile.
