...
sub executable_segments {
   ...
   my @segments;
   for my $line (split(/\n/,$map)) {
      next unless $line =~ /^\s+(CODE(?:\.__\S+)?|RENDERER_CODE)\s+load=\$([0-9A-Fa-f]{4})\s+size=\$([0-9A-Fa-f]{4})\b/;
...
sub scan_profile {
   ...
   for my $required ('CODE','RENDERER_CODE','CODE.__vcsc_function$vcs_standard_prepare_object_masks',
...
my $object_map=File::Spec->catfile($tmp,'standard_4k_ntsc_renderer.map');

my ($rc,$sig,$out,$err)=capture(
   $assembler,'-I',$profile,"--map=$object_map",'-o',$object,$renderer);
$rc==0 && !$sig or die "plain renderer assembly failed\nstdout:\n$out\nstderr:\n$err";
without_cartridge_usage($out) eq '' or die "plain renderer assembly wrote stdout:\n$out";
$err eq '' or die "plain renderer assembly wrote stderr:\n$err";
my $omap=read_file($object_map);
my %layout=(
   repostable=>map_symbol($omap,'vcs_standard_renderer_drawscreen::@repostable'),
...
my $bad_renderer=File::Spec->catfile($tmp,'standard_4k_ntsc_illegal_probe.s26');
my $bad_text=read_file($renderer);
...
print "vcs_standard_renderer_legal_bytes ok\n";
