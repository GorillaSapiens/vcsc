...
for my $tiny_case ([128,0x18],[256,0x19],[512,0x20]) {
   my($size,$value)=@$tiny_case;
   my $origin=0x10000-$size;
   my $tiny=chr(0xEA)x$size;
   substr($tiny,0,5,pack('C*',0xA9,$value,0x4C,$origin&0xFF,($origin>>8)&0xFF));
   substr($tiny,$size-6,6,pack('v3',$origin,$origin,$origin));
   write_raw(File::Spec->catfile($stella_in,"plain${size}b.bin"),$tiny);
}
...
for my $size (128,256,512) {
   $socmp =~ /plain${size}b\.bin: mapper vcsc=${size}B stella=${size}B MATCH\n/
      or die "Stella mapper comparison lost normalized ${size}B mapper match:\n$socmp";
}
...
# failure policy for genuine mismatches, not the mapper normalization itself.
my $stella_strict_out=File::Spec->catdir($tmp,'stella-compare-strict-out');
make_path($stella_strict_out);
