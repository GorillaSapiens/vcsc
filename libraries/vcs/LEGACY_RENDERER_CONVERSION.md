...
# legacy BASIC renderer conversion inventory

## Verdict

Conversion is feasible, but it is not a sensible "translate 2,000 lines of
assembly into C-like source" project. The scanline renderers are cycle-counted
...
4. add optional renderer features only after a minimal 4K standard-renderer image is
   byte- and timing-stable.

The standard renderer is a realistic first target. The multisprite renderer is also
...
| `standard/` | 8 | 2,587 | Standard and vertically reflected renderers, overscan, playfield drawing/scrolling, lives/status-bar helpers |
| `multisprite/` | 9 | 1,557 | Six-player multiplexing renderer, setup/sort logic, playfield helpers, normal/Superchip manifests |

The principal cycle-critical files are:

- `standard/std_renderer.asm` — 813 lines
- `standard/std_renderer_vertical_reflect.asm` — 771 lines
- `multisprite/multisprite_renderer.asm` — 1,094 lines

The `.inc` files are mostly concatenator manifests, not ordinary assembler
includes. They list generated application fragments and renderer pieces in the
...
Across all retained historical renderers, unofficial `DCP` appears 22 times,
...
The maintained all-five contract lives under `renderers/standard_4k_ntsc/`.
...
A separate `renderers/standard_4k_ntsc_playercolors/` contract restores the
...
No universal `$80`-based renderer ABI is being created. Vertical-reflect,
...
`vcs_standard_renderer_drawscreen()` entry. The module then owns sync,
...
Both maintained standard profiles expose one optional stack-accounted void overscan hook. The renderer provides a weak no-op fallback; a strong application definition runs after persistent object state and S are restored with `VBLANK` asserted and the overscan timer active.
...
- the non-reflected standard renderer;
- default asymmetric playfield and default decimal score;
- no optional playfield-color/height tables, status bar, paddle path, debug path, or mini-renderer. Player-row colors exist only in the separate no-missile profile.
...
renderer score paths. The selected module owns its `bcd24_t` score object through
...
The maintained `renderers/standard_4k_ntsc/` contract selects one exact
...
`renderers/standard_4k_ntsc/normalize.pl` now reads only the selected retained
inputs and deterministically generates:

- `standard_4k_ntsc_macros.inc`, containing deliberate ports of all five macros;
- `standard_4k_ntsc_renderer.s26`, containing the active overscan/visible renderer and
...
assembles the renderer without unofficial-opcode mode, verifies its current
...
### Phase 3 — minimal standard-renderer cartridge (complete)
...
### Phase 5 — grow standard-renderer coverage (in progress)
...
than as undocumented switches on one supposedly universal renderer.
