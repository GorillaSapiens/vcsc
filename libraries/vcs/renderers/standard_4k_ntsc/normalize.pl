...
my %input = (
   ...
   renderer   => File::Spec->catfile($legacy, 'standard', 'std_renderer.asm'),
);
...
require_source('renderer', qr/^renderer\b/m, 'the non-reflected renderer entry');
require_source('renderer', qr/^\s*RETURN\s*$/m, 'the selected renderer return');
...
sub generated_header {
   ...
   for my $key (qw(macro header overscan renderer score)) {
...
my %symbol = (
   ...
   stack1         => 'vcs_standard_renderer_scratch + 0',
   stack2         => 'vcs_standard_renderer_scratch + 1',
   PF1L           => 'PF1',
   PF2L           => 'PF2',
   PF1R           => 'PF1',
   PF2R           => 'PF2',
...
sub renderer_output {
   ...
   my $renderer = selected_lines($input{renderer}, $text{renderer});
   my %labels = collect_labels($overscan, $renderer);
   ...
   push @out, '';
   push @out, '.export vcs_standard_renderer_drawscreen';
   push @out, '.export __abimeta$V1$function$definition$vcs_standard_renderer_drawscreen$summary$paramsQ3D0$parametersQ3D0';
   push @out, '__abimeta$V1$function$definition$vcs_standard_renderer_drawscreen$summary$paramsQ3D0$parametersQ3D0 = 0';
   push @out, '.export __abimeta$V1$function$definition$vcs_standard_renderer_drawscreen$return$modeQ3Dreturn_voidQ3BvoidQ28szQ3D0Q29$return_voidQ20voidQ28sizeQ3D0Q29';
   push @out, '__abimeta$V1$function$definition$vcs_standard_renderer_drawscreen$return$modeQ3Dreturn_voidQ3BvoidQ28szQ3D0Q29$return_voidQ20voidQ28sizeQ3D0Q29 = 0';
   push @out, '.export __sbpmeta$F$vcs_standard_renderer_drawscreen';
   push @out, '__sbpmeta$F$vcs_standard_renderer_drawscreen = 0';
   push @out, '.export __sbpmeta$E$vcs_standard_renderer_drawscreen$vcs_standard_overscan_hook';
   push @out, '__sbpmeta$E$vcs_standard_renderer_drawscreen$vcs_standard_overscan_hook = 0';
   push @out, '';
   push @out, '.segment "RENDERER_CODE"';
   push @out, '.proc vcs_standard_renderer_drawscreen';
   ...
   push @out, '';
   push @out, '; ---- normalized from standard/std_renderer.asm ----';
   push @out, translate_source_set($renderer, \%labels);
   push @out, '.endproc';
   push @out, '.align 256';
   push @out, '';
...
ASM
   ...
   # timer still owns the visible-renderer start.
...
ASM
   ...
   # The retained renderer biases X by $54 so its sign bit doubles as the row-loop
...
ASM
   ...
   # first SLEEP immediately under @lastrendererline.
   my $last_pad_count = ($result =~ s/(\@lastrendererline:\n)             SLEEP 10/$1             SLEEP 6/);
   $last_pad_count == 1
...
; direct BCS to @lastrendererline keep these stubs off every fall-through path.
@skipDrawP0:
     lda #0
     tay
     jmp @continueP0
...
ASM
   ...
   my $steady_pad_count = ($result =~ s/(\@continuerenderer:\n)     SLEEP 2\n/$1/);
   $steady_pad_count == 1
...
@startrenderer:
     ...
     cpy vcs_standard_renderer_scratch + 3 ;3, exclusive height
     bcc @drawP1 ;3 when drawing, 2 when skipped
     lda vcs_standard_renderer_scratch + 4 ;3, permanent zero
     jmp @continueP1 ;3
@drawP1:
...
ASM
   my $new_player0 = <<'ASM';
     ...
     cpy vcs_standard_renderer_scratch + 2 ; exclusive height
     bcc @drawP0
     lda vcs_standard_renderer_scratch + 4 ; permanent zero
     jmp @continueP0
@drawP0:
     lda (vcs_standard_player0_graphics),y
@continueP0:
...
@prepare_player_state:
     ...
     clc
     adc #1
     sta vcs_standard_renderer_scratch + 2
     lda vcs_standard_player1_height
     clc
     adc #1
     sta vcs_standard_renderer_scratch + 3
     lda #0
     sta vcs_standard_renderer_scratch + 4
     rts
...
ASM
   ...
   # instructions in the visible renderer. LDA preserves carry, so loading 1 and
   ...
   for my $old_offset (sort keys %q3_scratch_offset) {
      ...
      my $count = ($result =~ s/vcs_standard_renderer_scratch \+ $old_offset/vcs_standard_object_masks + $new_offset/g);
   ...
   my $q3_import_count = ($result =~ s/vcs_standard_renderer_scratch/vcs_standard_object_masks/g);
   $q3_import_count == 1
...
my %generated = (
   ...
   'standard_4k_ntsc_renderer.s26'   => renderer_output(),
);
