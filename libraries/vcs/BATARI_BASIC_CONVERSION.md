...
# batari Basic kernel conversion inventory
...
The snapshot is reference material rather than a standalone batari Basic build.
...
explicit ABI profile that says which batari bytes are:
...
Merely declaring `ref` aliases for the batari addresses is insufficient: the
...
The batari startup clears RAM by driving the hardware stack through memory,
...
kernel-specific startup that initializes the batari ABI state and any VCSC data
...
The current production linker profile is an unbanked 4K image. The batari
...
1. Keep `libraries/vcs/batari-basic/` byte-for-byte upstream.
