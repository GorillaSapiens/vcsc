...
- `CV/mapper.c26`, `CV/ram.c26` ... CommaVid CV fixed 2K ROM plus shared 1K split-address cartridge RAM; linked `.map` output supplies the simulator topology
...
- `*/bankcall.s26` ... mapper-local maintained sources for selector-controlled automatic cross-bank calls; the public ABI is defined in [`../../BANKSWITCHING.md`](../../BANKSWITCHING.md) and uses mapper-defined one-byte bank-call descriptors rather than inferring bank identity from logical PCs. F8/F8SC/F6/F6SC/F4/F4SC, FA, DPC, FA2-24/28, JANE, 0840, UA, UASW, 0FA0, and WD consume that descriptor ABI now.
- `*/entry.s26` ... mapper-local variable-length reset-entry sources for the migrated descriptor-ABI mappers. Selector-normalizing entries use raw `op0C` absolute-NOP syntax without depending on `--illegal`; zero-length entries are valid when no mapper normalization is required (3F). The linker replicates the maintained bytes ahead of each ordinary vector handler.
...
- `3E/mapper.c26` ... 3E ROM/RAM extension of the same 2K-window family, with a VCSC 256 x 1K swap-RAM pool; see `3E/README.md` for Stella compatibility limits
...
- `heart_score_component.c26` ... fixed-footprint 0..11 heart meter with half-heart steps and an optional separately colored 0..11 playfield-box capacity background; seven visible lines, 12-pixel heart pitch, exact full/half left-prefix geometry
...
88,100,112,124,136 in 160-pixel TIA coordinates. Smaller values are exact
left-justified prefixes; existing hearts never slide as the value changes.

The same seven scanlines can optionally show a separately colored capacity box
behind each heart slot. `boxes` is the capacity from 0 through 11 and
`box_color` is the single playfield color used by all enabled boxes. Each box is
8 pixels wide with the same 12-pixel pitch as the hearts. Health is clamped to
capacity during VBLANK, so a half-heart is cleared when it would extend beyond
the final box.

```vcsc
instantiate "heart_score_component.c26" as health

health_score := 5;      // full hearts
health_half := 1;       // add the left half of the next heart
health_boxes := 8;      // capacity: 0..11
health_color := 0x46;
health_box_color := 0xce;
```

The renderer uses P0/P1 for hearts and the playfield for boxes. The playfield
stream is asymmetric: each row loads box-count-specific PF1/PF2 bytes for the
left half, then switches to the corresponding right-half bytes at fixed cycle
positions. The heart pixel schedule remains the established Jentzsch-derived
geometry. Half states through 7.5 use looped SAX-assisted player writes; the
dense 8.5, 9.5, and 10.5 paths are unrolled so the half-heart write and
playfield transition both fit without moving established pixels. Score 11 is
the maximum; there is no 11.5 geometry.

One instance uses **16 RIOT-RAM bytes** total, including public `score`, `half`,
`color`, `boxes`, and `box_color`, the renderer pointer, cached TIA setup, and
four cached asymmetric playfield bytes. Horizontal player placement is
re-established by `draw()` itself rather than being left as VBLANK state for a
later visible component to preserve.

The component consumes exactly **seven visible scanlines**, enters `draw()` at
cycle 3, and returns at cycle 0 after its terminal `WSYNC`. It owns P0/P1,
PF0/PF1/PF2, `COLUPF`, and reflected `CTRLPF` while drawing. Terminal cleanup
clears the playfield, but does not restore the caller's `COLUPF`/`CTRLPF`; a
following visible component must establish its own playfield state. Half-heart
pixels use no Ball or missile graphics.

Stella 7.0 certification preserves all historical full/half heart rasters when
the box color is hidden against the background, and separately locks the
visible box geometry for every capacity 0..11. The public example directory
[`examples/01_basic/14_heart_score`](../../examples/01_basic/14_heart_score/)
contains F8 composition cartridges with the maintained 181-line `player_color`
renderer above and below the meter. Right joystick UP/DOWN changes health by a
half-heart; LEFT/RIGHT changes capacity by one box. Left joystick moves the
selected gameplay object and SELECT cycles P0, P1, and Ball. Four blank lines
make 181 + 4 + 7 = 192 visible scanlines.
...
```sh
vcsc -I libraries/vcs libraries/vcs/F8/mapper.c26 source.c26 -o game-f8.bin
vcsc -I libraries/vcs libraries/vcs/F6/mapper.c26 source.c26 -o game-f6.bin
vcsc -I libraries/vcs libraries/vcs/F4/mapper.c26 source.c26 -o game-f4.bin
```
...
No VCS linker cfg is required: the included C26 profile supplies the cartridge
and allocator topology directly.
...
- `2K/mapper.c26` describes a 2048-byte cartridge linked at `$F800-$FFFF`, with vectors in its final six bytes; pass or include that C26 profile explicitly.
- `4K/mapper.c26` describes the standard 4K cartridge mapped at `$F000-$FFFF` with vectors at `$FFFA-$FFFF`; the driver compiles it automatically when no `-T` is supplied.
- The 4KSC, F8/F6/F4, 0840, UA/UASW, 0FA0, E0, FE, WD, DPC, 3F/3E, JANE, FA/RAM Plus, banked SC, and OMNI `.c26` profiles are installed together and emit their exact cartridge image sizes. Their linked `.map` sidecars carry the topology `vcsc-sim` needs; no VCS mapper cfg files are installed.
...
- `vcsc` discovers `4K/mapper.c26` in the source tree or installed `libraries/vcs` directory and uses it by default. Pass or include another C26 profile to select a different cartridge layout; reserve `-T` for generic linker scripts.
- The 128 physical RIOT RAM bytes are not double-counted. `vcs.c26` declares the full `$80-$FF` block, marks its stack backing, and `vcsc-ld` reserves the required top bytes dynamically from the whole-program source call graph before placing ordinary storage. The page-1 addresses `$0180-$01FF` are mirrors of `$80-$FF`, not separate RAM.
...
The public `CV/mapper.c26` profile emits one fixed 2K ROM image mapped at
`$F800-$FFFF`. `CV/ram.c26` exposes the common `cartram` qualifier as a
...
The public `0840/mapper.c26` profile emits two 4K physical banks in file order.
...
sidecar carries the decoded selector families; `vcsc-sim` models them on both
...
The public `UA/mapper.c26` and `UASW/mapper.c26` profiles emit two 4K physical
...
The linked `.map` sidecar provides the matching masked decoder and
selector-to-file-bank association to `vcsc-sim`. The final bank carries
`UA\0\0` or `UASW` at `$FFF8-$FFFB`.

### 0FA0 / Fotomania profile

The public `0FA0/mapper.c26` profile emits two 4K physical banks. Physical/file
...
The public `DPC/mapper.c26` profile emits the conventional 10,495-byte DPC
...
`DPC/registers.c26` declares the DPC register bindings. The diagnostic reads all 2K of
...
The public `FE/mapper.c26` profile emits two physical 4K chunks in released-cart
...
select bank 1. The linked `.map` sidecar gives `vcsc-sim` the matching delayed-bus model.
...
3, which is also logical bank0's top segment. The mapper-owned `WD/entry.s26` hook reads `$39` with raw `op0C`, waits
naturally through the delayed selector instruction, and then enters normal startup
with complete logical bank0/state1 mapping established.
...
logical bank, so public linking and simulation share the C26 topology.

### FA / RAM Plus profile and allocatable RAM

The public `FA/mapper.c26` profile emits three complete 4K physical banks.
...
`FA/ram.c26` declares the shared device as:
...
The public `4KSC/mapper.c26`, `F8SC/mapper.c26`, `F6SC/mapper.c26`, and
`F4SC/mapper.c26` profiles reserve
...
The SC profiles include `4KSC/ram.c26`, whose hardware-specific window definition
