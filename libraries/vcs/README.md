...
- `renderers/standard_4k_ntsc/` ... legacy monolithic all-five-object solid-color component whose generated assembly object carries its own placement, page, and hidden-stack contracts
...
- Current stack sizing accounts automatically for source-level JSR return addresses; ordinary generated calls push no compiler state. Assembly components use `.callstackextra` object metadata for calls, pushes, or stack-pointer use hidden from the source call graph. C26 renderer templates emit the same assembler directive through inline assembly, so their internal helper JSRs no longer depend on whichever cfg happens to link the application. The standard renderer exports its assembly-initiated overscan-hook edge and declares four supplementary bytes for the deeper internal mask-preparation chain. Its code and score-table layouts likewise carry startup-region, page-alignment, private-route, `.pagecontain`, and `.indexrange` facts in the object instead of a renderer-specific cfg. Arbitrary inline-assembly stack use must still be declared explicitly.
