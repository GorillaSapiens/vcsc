...
//! @brief Reset mutable assembler-time symbols before sequential o65 emission.
static void reset_mutable_symbols_for_o65(symtab_t *symbols)
{
   symbol_t *sym;
...
//! @brief Apply a .set statement while building o65 data buffers.
static int process_set_statement_o65(o65_writer_t *wr, const stmt_t *stmt)
{
   symbol_t *sym;
   ...
   if (rc != EXPR_EVAL_OK) {
      if (rc == EXPR_EVAL_UNRESOLVED)
         writer_error(wr->ctx, stmt, "could not evaluate .set expression for o65 output");
      else
                        ...
                        reloc_expr_info_t *out)
{
   ...
   switch (expr->kind) {
      ...
      case EXPR_UNARY:
         ...
         if (inner.is_reloc) {
            writer_error(wr->ctx, stmt, "o65 output does not support unary operators on relocatable expressions");
            return 0;
         }
      ...
      case EXPR_BINARY:
         ...
         if (expr->u.binary.op == EXPR_BOP_LOG_AND || expr->u.binary.op == EXPR_BOP_LOG_OR) {
            if (left.is_reloc) {
               writer_error(wr->ctx, stmt, "o65 output does not support logical operators on relocatable expressions");
               return 0;
            }
         ...
         switch (expr->u.binary.op) {
            ...
            case EXPR_BOP_LOG_OR:
               if (left.is_reloc || right.is_reloc) {
                  writer_error(wr->ctx, stmt, "o65 output does not support this operator on relocatable expressions");
                  return 0;
               }
               {
                  expr_eval_status_t rc;
...
//! @brief Decode the single quoted string expected by .echo/.error in o65 output.
static int directive_decode_message(o65_writer_t *wr,
                                    const stmt_t *stmt,
...
static int write_segment_stmt(o65_writer_t *wr, const stmt_t *stmt)
{
   ...
   if (stmt->kind == STMT_CONST && stmt->u.cnst.assign_kind == CONST_ASSIGN_SET)
      return process_set_statement_o65(wr, stmt);
      ...
      case STMT_DIR:
         ...
         if (!strcmp(stmt->u.dir->name, ".align")) {
            ...
            if (expr_eval(stmt->u.dir->exprs->expr, &wr->ctx->symbols, stmt->scope, stmt->file, stmt->address, &boundary) != EXPR_EVAL_OK || boundary <= 0) {
               writer_error(wr->ctx, stmt, "invalid .align boundary in o65 output");
               return 0;
            }
...
int o65_write_object_file(FILE *fp, asm_context_t *ctx)
{
   ...
   add_exports(&wr);
   reset_mutable_symbols_for_o65(&ctx->symbols);
