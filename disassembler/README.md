...
and reloads retain it, and mapper cartridge RAM uses a sparse path-local source
map whose read/write aliases collapse to the same physical byte. For split RAM,
a normal read alias preserves the stored source, a write to the read alias is an
ignored no-op, and a read from the write alias returns an unpredictable value
while overwriting that backing byte, so its prior provenance is destroyed. Unknown
absolute indexing invalidates every reachable write-alias byte; unresolved indirect
reads conservatively drop split-RAM provenance. The modeled cartridge-RAM families
currently include Superchip, FA/FA2, CV, WD, E7, 3E, and GameLine. At control-flow
joins a source survives only when every incoming state agrees, so provenance is
never manufactured by merging unrelated paths.

Known ROM-sourced bytes copied into RIOT RAM can become instruction bytes in the
same fixed-point analysis. Distinct generated byte patterns at the same RAM PC
are retained separately rather than prematurely merged; those RAM instructions
may change mapper state and transfer control back to ROM. The generated source
prints a comment-only `RIOT RAM execution discovered by mapper-local provenance` block with physical ROM file offsets
for each known opcode/operand source. Congo Bongo is the canonical real-world
case: separate feasible paths construct `LDA $FFF8 ; JMP $F0E0` and
`LDA $FFF9 ; JMP $D889` at `$00F0`, and those generated selector accesses are
ordinary mapper evidence rather than a game-specific signature. ROM provenance
...
loops are viable. Bus legality is distinct from whether an access changes state:
ordinary cartridge-ROM writes and writes to split-RAM read aliases are legal
ignored cycles, while reads from split-RAM write aliases are legal unpredictable
reads with the RAM-overwrite side effect described above. RMW is therefore legal
on either split alias. Mapper-defined accesses are judged before generic TIA/RIOT
decoding, and WD preserves both old/new continuations across its hardware-delayed
selector. Generated headers report live/dead/weak
...
`$F900-$F9FF`). RMW against either alias is legal but neutral mapper evidence:
read-alias RMW reads RAM and has ignored write phases; write-alias RMW starts with
an unpredictable read that overwrites RAM before the CPU write phases.
...
is legal and diagnostic only; it neither establishes nor contradicts 3E.
...
signature. Superchip, CBS RAM Plus/FA, CommaVid CV, WD, E7, and 3E expose
separate CPU aliases for the same physical RAM. Both aliases are legal bus
sources/destinations, but their side effects differ: a normal read alias returns
stored RAM; writing that alias is ignored; reading the write alias returns an
unpredictable read-from-write-port value and overwrites the backing byte; writing
the write alias stores normally. Consequently a 6502 RMW is legal on either
alias. It is retained as a diagnostic count but is neutral mapper evidence—never
a veto and never positive family proof. Only pure direction-correct accesses may
contribute positive RAM evidence, and CV/WD still require independent family
evidence so a hypothetical RAM overlay cannot prove itself from accesses it
reinterprets. Provenance is invalidated whenever an exact or finite-index write
may alter a backing RAM byte; unresolved indirect destructive accesses drop the
affected split-RAM provenance conservatively.
...
RMW anywhere in `$x000-$x0FF` is legal and diagnostic only; it does not complete
the pure read/write evidence needed for automatic SC promotion. Established
RESET/control flow into the SC write port `$x000-$x07F` still vetoes semantic
promotion because instruction fetches would hit the write alias rather than ROM.
The read port is not the same execution veto because code may deliberately run
from populated cartridge RAM. Explicit VCSC SC signatures remain authoritative.
...
TIA `$30-$3F` and 3E/3F selector writes. For detached speculative validation, a pure store also needs a meaningful
state-changing sink in the established hardware model: RIOT RAM, valid TIA/RIOT
registers, cartridge RAM write aliases, coprocessor registers, and mapper-triggered
writes qualify. A legal but ignored store to mapped cartridge ROM or a split-RAM
read alias does not validate an island. RMW is not rejected merely because its
write phase is ignored when its read phase has a real source. This sink rule is
separate from A5 bus legality. Configuration-dependent cartridge RAM is checked
against carried mapper state; the earlier structural prefilter remains conservative
when that state is not yet known.
...
straight-line dependency path--raw bytes are written one row per line. Static
graphics recovery only treats addresses that are definitely ROM under the
selected mapper as ROM sources; Superchip/native split RAM and configuration-
dependent E7/3E/GL/CM overlays cannot expose hidden physical ROM as graphics.
The analysis follows register transfers and simple ALU/accumulator-shift
transforms, and recognizes `STA`, `STX`, and `STY` graphics writes. The assembler's actual binary
...
Mapper support beyond unbanked/F8/F6/F4/Superchip/FA/FA2/CV/E0/GL/CM/E7/3F/3E/FE/JANE/0840/UA/UASW/0FA0/DPC/WD/WDSW/FC is deliberately conservative.
DPC+, CDF/CDFJ/CDFJ+ and other coprocessor cartridges need separate mapper models rather than being mislabeled as supported families.

CompuMate (CM) uses controller port A as cartridge control rather than a normal
ROM hotspot: SWCHA D1:D0 select one of four 4K ROM banks, bank 3 powers up, D4
selects ROM versus the 2K RAM overlay at $1800-$1FFF, and D5 controls RAM write
versus read direction. Automatic detection requires a repeated SWCHA read/write
protocol plus SWACNT traffic so an ordinary joystick poll cannot self-identify as
CM.
