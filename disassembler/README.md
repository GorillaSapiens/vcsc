...
map whose read/write aliases collapse to the same physical byte. The modeled
cartridge-RAM families currently include Superchip, FA/FA2, CV, WD, E7, 3E, and
GameLine. At control-flow joins a source survives only when every incoming state
agrees, so provenance is never manufactured by merging unrelated paths.
