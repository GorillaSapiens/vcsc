...
static void usage(FILE *fp)
{
   fprintf(fp,
      ...
      "It invokes n65c, n65asm, and n65ld as needed.\n"
      "\n"
      "Overall options:\n"
      ...
      "  -Wc,ARG,...          Pass comma-split args to n65c\n"
      "  -Wa,ARG,...          Pass comma-split args to n65asm\n"
      "  -Wl,ARG,...          Pass comma-split args to n65ld\n"
      "  -Xcompiler ARG       Pass one extra arg to n65c\n"
   ...
   char *vcs_cfg_path, size_t vcs_cfg_sz)
{
   ...
   build_installed_tool_path(cc_inst, sizeof(cc_inst), self_path, "n65c");
...
static void temp_store_make_dir(temp_store_t *ts)
{
   if (ts->made_tempdir)
      return;
   snprintf(ts->tempdir, sizeof(ts->tempdir), "/tmp/n65cc.XXXXXX");
...
static int print_companion_version(const char *name, const char *path, size_t size)
{
   ...
   printf("%-6s : %-*s : ", name, (int)size, path);
   fflush(stdout);
   ...
   const char *ar_path, const char *sim_path)
{
   ...
   printf("%-6s : %-*s : %s\n", "n65cc", (int)size, self_path, VERSION);
   rc |= print_companion_version("n65c", cc_path, size);
   rc |= print_companion_version("n65asm", as_path, size);
   rc |= print_companion_version("n65ld", ld_path, size);
   rc |= print_companion_version("n65ar", ar_path, size);
   rc |= print_companion_version("n65sim", sim_path, size);
   return rc;
}
...
static bool defines_have_consumer(const driver_options_t *opt)
{
   ...
   for (i = 0; i < opt->inputs.count; ++i) {
      ...
      if (opt->compile_only) {
         if (kind == INPUT_N || kind == INPUT_ASM)
            return true;
         continue;
      }
...
int main(int argc, char **argv)
{
   ...
   for (i = 0; i < opt.inputs.count; ++i) {
      ...
      if (opt.asm_only) {
         ...
         run_cc(cc_path, &opt, nlib_inc, in->path, asm_path);
         continue;
      }
