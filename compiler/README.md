...
Condition truth-value evaluation, two-operand comparisons, discarded non-call expression results, simple assignments into fixed symbols, casts, local/global absolute-reference conversion, built-in increment/decrement, and the main binary operator families use fixed per-site BSS scratch. Current prefixes include `__n65_truthtmp_N`, `__n65_comparetmp_N`, `__n65_discardtmp_N`, `__n65_assigntmp_N`, `__n65_addtmp_N`, `__n65_shifttmp_N`, `__n65_binarytmp_N`, `__n65_ptrdifftmp_N`, `__n65_casttmp_N`, `__n65_abslocaltmp_N`, `__n65_absglobaltmp_N`, and `__n65_incdectmp_N`. The compiler temporarily redirects `fp` while lowering each expression, restores it using the 6502 hardware stack, and copies the converted result back into the caller destination. These paths never invoke `_pushN` or `_popN`. Scratch is still allocated per expression site and is not overlaid; remaining compound-assignment, pointer/index lvalue, initializer, bit-field helper, and switch paths still use the transitional software stack.
